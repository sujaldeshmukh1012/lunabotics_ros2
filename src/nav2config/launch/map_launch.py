...
def generate_launch_description():
	...
	"map_file",
	default_value=PathJoinSubstitution([FindPackageShare("nav2config"), "maps", "my_map1.yaml"]),
	description="Full path to the yaml map file",
    )

    amcl_params_file_arg = DeclareLaunchArgument(
	"amcl_params_file",
	default_value=PathJoinSubstitution([FindPackageShare("nav2config"), "config",  "navigation.yaml"]),
	description="Full path to the ROS2 amcl params file",
    )
